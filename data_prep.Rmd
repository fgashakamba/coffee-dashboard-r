...
pacman::p_load(magrittr, dplyr, stringr, tidyr, lubridate,
               sf, googlesheets4, openssl, janitor)
...
data_farmers %<>% left_join(data_farms_stats, by = "national_id")

# Load and process geospatial data
# Load and process geospatial data using reliable relative paths
country <- st_read("data_wgs84/RW_country.gpkg", layer = "country") %>% 
  st_zm(drop = TRUE, what = "ZM") %>%  st_make_valid() %>% 
  st_transform(crs = 32736)
lakes <- st_read("data_wgs84/RW_lakes.gpkg", layer = "lakes") %>% 
  st_zm(drop = TRUE, what = "ZM") %>%  st_make_valid() %>% 
  st_transform(crs = 32736)
np <- st_read("data_wgs84/RW_national_parks.gpkg", layer = "np") %>% 
  st_zm(drop = TRUE, what = "ZM") %>%  st_make_valid() %>% 
  st_transform(crs = 32736)
districts <- st_read("data_wgs84/RW_districts.gpkg", layer = "districts") %>% 
  st_zm(drop = TRUE, what = "ZM") %>%  st_make_valid() %>% 
  st_transform(crs = 32736) %>%
...
# This file will be read by the Shiny app
app_data %>% saveRDS("coffee_dashboard_data.rds")
```

**Downloading farmers data
**Note:** The above code is designed to be run in an R Markdown document. It fetches data from Google Sheets, processes it, and saves the final datasets into a single `.rds` file for use in a Shiny app. 